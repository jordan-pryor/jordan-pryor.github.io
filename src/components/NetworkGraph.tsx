    ...
    onMount(() => {
      ...
      fetch(`https://api.github.com/users/${githubUsername}/events`)
        ...
        .then(events => {
          ...
          const xAxis = d3.axisBottom(xAxisScale).ticks(31).tickFormat(d => d + 1).tickSize(0);
          const yAxis = d3.axisLeft(yAxisScale).ticks(5).tickFormat(d => d + 1).tickSize(0);
  
          svgBlocks.append("g")
            .attr("class", "x-axis")
            .attr("transform", `translate(20, ${blockHeight})`)
            .call(xAxis)
            .selectAll("text")
            .attr("fill", colors.label);
          ...
          // Color Key
          const colorKeyContainer = d3.select(graphContainer).append("div")
            .style("position", "relative")
            .style("width", "100%")
            .style("height", "100px")
            .style("margin-top", "10px");
  
          const colorKeySvg = d3.select(colorKeyContainer.node())
            .append("svg")
            .attr("width", width - 40)
            .attr("height", 100);
